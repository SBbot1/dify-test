...
#### Image URL
You only need to pass the URL of the image, andSuperBotwill automatically download the image and return it to the user.
...
- `meta` The metadata of the file, if you know the type of the file, it is best to pass a `mime_type`, otherwiseSuperBotwill use `octet/stream` as the default type
...
This tool takes in an user_id and the text to be summarized, and returns the summarized text.SuperBotwill use the default model of the current workspace to summarize the long text.
...
This tool takes in web page link to be crawled and a user_agent (which can be empty), and returns a string containing the information of the web page. The `user_agent` is an optional parameter that can be used to identify the tool. If not passed,SuperBotwill use the default `user_agent`.
