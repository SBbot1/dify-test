const translation = {
  ...
  form: {
    ...
    desc: "ナレッジの説明",
    descInfo:
      "ナレッジの内容を概説するための明確なテキストの説明を書いてください。この説明は、複数のナレッジから推論を選択する際の基準として使用されます。",
    descPlaceholder:
      "このナレッジに含まれる内容を説明してください。詳細な説明は、AIがナレッジの内容にタイムリーにアクセスできるようにします。空の場合、Superbotはデフォルトのヒット戦略を使用します。",
    ...
    indexMethodHighQualityTip:
      "ユーザーのクエリ時により高い精度を提供するために、OpenAIの埋め込みインターフェースを呼び出して処理します。",
    indexMethodEconomy: "経済的",
    indexMethodEconomyTip:
      "オフラインのベクトルエンジン、キーワードインデックスなどを使用して精度を低下させることなく、トークンを消費せずに処理します。",
