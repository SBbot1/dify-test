const translation = {
  ...
  account: {
    ...
    notEqual: "两个密码不相同",
    confirmPassword: "确认密码",
    langGeniusAccount: "Superbot 账号",
    langGeniusAccountTip: "您的SuperBot账号和相关的用户数据。",
    editName: "编辑名字",
    showAppLength: "显示 {{length}} 个应用",
  },
  members: {
    ...
    invitedAsRole: "邀请为{{role}}用户",
    invitationSent: "邀请已发送",
    invitationSentTip: "邀请已发送，对方登录SuperBot后即可访问你的团队数据。",
    invitationLink: "邀请链接",
    failedinvitationEmails: "邀请以下邮箱失败",
    ok: "好的",
    removeFromTeam: "移除团队",
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: "体验",
      exhausted: "超出限额",
      desc: "托管 OpenAI 由SuperBot提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。",
      callTimes: "调用次数",
  ...
  apiBasedExtension: {
    title:
      "API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在SuperBot上的各类应用中直接使用。",
  ...
  about: {
    changeLog: "更新日志",
    updateNow: "现在更新",
    nowAvailable: "Superbot {{version}} 现已可用。",
